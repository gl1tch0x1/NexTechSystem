...
import {
  verifyAdminPin,
  DEFAULT_SYSTEM_ADMIN_PIN_HASH,
} from "../utils/admin-pin.js";
    ...
    if (!slots) {
      res
        ...
        .json({
          ...
          error: {
            code: "BAD_REQUEST",
            message: "Component slots payload is required.",
          },
        });
      return;
    }
...
export class WalletController {
  ...
  async addFunds(req: AuthenticatedRequest, res: Response): Promise<void> {
    if (ENV.NODE_ENV !== "development" || process.env.ENABLE_DEMO_WALLET_TOPUP !== "true") {
      res.status(501).json({ success: false, error: {
        code: "PAYMENT_PROVIDER_REQUIRED",
        message: "Wallet top-ups are unavailable until a verified payment provider is connected.",
      } });
      return;
    }
    if (!req.user) {
    ...
    if (!Number.isFinite(num) || num <= 0 || num > 50000) {
      res
        ...
        .json({
          ...
          error: {
            code: "INVALID_AMOUNT",
            message: "Amount must be a positive number up to 50,000 AED.",
          },
        });
      return;
    }
    ...
    if (adminPin || num > 2500) {
      ...
      const userPinHash =
        req.user.adminPinHash || DEFAULT_SYSTEM_ADMIN_PIN_HASH;
      if (!pinToVerify || !verifyAdminPin(pinToVerify, userPinHash)) {
        res.status(403).json({
          ...
          error: {
            ...
            message:
              "Secondary Security PIN verification failed. Please enter the valid authorization PIN.",
          },
        });
        return;
      }
    ...
    const result = await walletService.creditWallet({
      ...
      reason:
        num > 2500
          ? "Authorized High-Value Wallet Injection (PIN Verified)"
          : "Direct Customer Wallet Top-up (Demo Sandbox)",

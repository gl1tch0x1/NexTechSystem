...
export class CurrencyService {
  ...
  public static async refreshRatesFromApi(): Promise<void> {
    ...
    try {
      let res: any;
      try {
        res = await fetch(this.PRIMARY_API, {
          signal: AbortSignal.timeout(4000),
        });
      } catch {
        res = await fetch(this.FALLBACK_API, {
          signal: AbortSignal.timeout(4000),
        });
      }
      ...
      if (res && res.ok) {
        ...
        if (aedRates && typeof aedRates === "object") {
          for (const [code, info] of Object.entries(SUPPORTED_CURRENCIES)) {
            ...
            if (
              aedRates[lowerCode] &&
              typeof aedRates[lowerCode] === "number"
            ) {
          ...
          console.log(
            `[CurrencyService] Live rates synchronized from fawazahmed0/exchange-api (USD: ${SUPPORTED_CURRENCIES.USD.rateAgainstAED}, EUR: ${SUPPORTED_CURRENCIES.EUR.rateAgainstAED}, SAR: ${SUPPORTED_CURRENCIES.SAR.rateAgainstAED})`,
          );
        }
      }
    } catch (err: any) {
      console.warn(
        "[CurrencyService] Failed to refresh live rates, using fallback rates:",
        err?.message,
      );
    }
  }
  ...
  public static convertFromAED(
    amountInAED: number,
    targetCurrency: string,
  ): number {
    const currency =
      SUPPORTED_CURRENCIES[targetCurrency.toUpperCase()] ||
      SUPPORTED_CURRENCIES.AED;
    return Number(
      (amountInAED * currency.rateAgainstAED).toFixed(currency.decimals),
    );
  }

  public static convertToAED(
    amountInTarget: number,
    sourceCurrency: string,
  ): number {
    const currency =
      SUPPORTED_CURRENCIES[sourceCurrency.toUpperCase()] ||
      SUPPORTED_CURRENCIES.AED;

...
export class PricingService {
  async calculateOrderTotals(params: {
    ...
    for (const reqItem of params.items as Array<{
      productId: string;
      quantity: number;
      variantId?: string;
    }>) {
      ...
      if (!Number.isInteger(qty) || qty <= 0 || qty > 999) {
        throw new Error(
          `Invalid item quantity for product ${reqItem.productId}. Must be a positive integer between 1 and 999.`,
        );
      }
      ...
      if (
        ...
        const variant = product.variants.find(
          (v: any) => v.id === reqItem.variantId,
        );
        if (variant) {
          effectivePrice = variant.price;
          effectiveSalePrice =
            variant.salePrice ||
            (variant.compareAtPrice ? variant.price : product.salePrice);
      ...
      verifiedItems.push({
        ...
        variantTitle:
          reqItem.variantId && product.variants
            ? product.variants.find((v: any) => v.id === reqItem.variantId)
                ?.title
            : undefined,
    ...
    const discountedTaxableSubtotal = Math.max(
      0,
      taxableItemsSubtotal - couponDiscount * taxableRatio,
    );
    const tax = isZeroRated
      ? 0
      : Math.round(discountedTaxableSubtotal * (taxRate / 100) * 100) / 100;

    const preWalletTotal =
      Math.round((discountedSubtotal + tax + shippingFee) * 100) / 100;

    // Wallet deduction verification
    let walletAmountUsed = 0;
    if (
      params.requestedWalletDeduction &&
      params.requestedWalletDeduction > 0
    ) {
      const availableWallet = Math.min(
        params.requestedWalletDeduction,
        params.userWalletBalance || 0,
      );
    ...
    const total = Math.max(
      0,
      Math.round((preWalletTotal - walletAmountUsed) * 100) / 100,
    );

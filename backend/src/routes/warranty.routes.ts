...
router.get('/verify/:serial', async (req: Request, res: Response): Promise<void> => {
  try {
    const rawParam = req.params.serial;
    const rawSerial = (Array.isArray(rawParam) ? rawParam[0] : rawParam)?.trim();
    if (!rawSerial || rawSerial.length < 3 || rawSerial.length > 64) {
      res.status(400).json({
        success: false,
        error: { message: 'A valid hardware serial number (between 3 and 64 characters) is required.' },
      });
      return;
    }
      ...
      if (cleanSerial.includes('14900') || cleanSerial.includes('4090') || cleanSerial.startsWith('SN-')) {
        ...
        res.json({
          ...
          data: {
            ...
            daysRemaining: Math.max(0, Math.ceil((new Date(expiryDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24))),
    ...
    const expiryTime = purchaseTime + (warrantyMonths * 30.5 * 24 * 60 * 60 * 1000);
    ...
    const maskedOrder = orderNum.length > 8
    ...
    res.status(500).json({ success: false, error: { message: err.message || 'Internal warranty service error.' } });
  }
});

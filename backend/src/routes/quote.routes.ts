...
import { apiLimiter, adminLimiter } from '../middlewares/rate-limiter.middleware.js';
...
router.post('/', apiLimiter, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!companyName || !contactName || !contactEmail) {
      res.status(400).json({
        success: false,
        error: { message: 'Company name, contact name, and official business email are required.' },
      });
      return;
    }
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
      res.status(400).json({
        success: false,
        error: { message: 'At least one hardware line item is required for corporate quotation.' },
      });
      return;
    }
    ...
    if (items.length > 50) {
      res.status(400).json({
        success: false,
        error: { message: 'Maximum 50 line items allowed per single quotation request.' },
      });
      return;
    }
    ...
    for (const rawItem of items) {
      ...
      processedItems.push({
        productId: String(rawItem.productId || `PROD-${Date.now()}`).slice(0, 64),
        productName: String(rawItem.productName || 'Enterprise Hardware Component').slice(0, 150),
    ...
    res.status(500).json({ success: false, error: { message: err.message || 'Failed to submit quote request.' } });
  }
});
...
router.get('/', adminLimiter, authenticate, requireRole('ADMIN'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ success: false, error: { message: err.message || 'Failed to fetch quotes.' } });
  }
});

// 3. Get Single Quote (Admin Only - OWASP A01 Defense)
router.get('/:id', adminLimiter, authenticate, requireRole('ADMIN'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const id = getParamId(req.params.id);
    const quote = await quoteRepository.findById(id);
    if (!quote) {
      res.status(404).json({ success: false, error: { message: 'Quote not found.' } });
      return;
    }
    res.json({ success: true, data: quote });
  } catch (err: any) {
    res.status(500).json({ success: false, error: { message: err.message || 'Failed to fetch quote.' } });
  }
});

// 4. Update Quote (Admin Only - OWASP A01 Defense)
router.put('/:id', adminLimiter, authenticate, requireRole('ADMIN'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const id = getParamId(req.params.id);
    ...
    const quoteDiscount = discount !== undefined ? Math.max(0, parseFloat(discount) || 0) : existing.discount;
    const taxableAmount = Math.max(0, subtotal - quoteDiscount);
    const vatRate = 0.05;
    const tax = Math.round(taxableAmount * vatRate * 100) / 100;
    const shippingFee = shipping !== undefined ? Math.max(0, parseFloat(shipping) || 0) : existing.shipping;
    ...
    const updated = await quoteRepository.update(id, {
      ...
      total,
      notes: notes !== undefined ? String(notes).slice(0, 1000) : existing.notes,
    ...
    try {
      await auditService.log({
        ...
        resource: 'quotes',
        resourceId: id,
        details: { quoteNumber: existing.quoteNumber, status: status || existing.status, total },
      });
    } catch {}
    ...
    res.status(500).json({ success: false, error: { message: err.message || 'Failed to update quote.' } });
  }
});

// 5. One-Click Convert Quote to Verified Sales Order (Admin Only - OWASP A01 Defense)
router.post('/:id/convert', adminLimiter, authenticate, requireRole('ADMIN'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const id = getParamId(req.params.id);
    const quote = await quoteRepository.findById(id);
    if (!quote) {
    ...
    if (quote.status === 'CONVERTED') {
      res.status(400).json({
        success: false,
        error: { message: `This quotation was already converted to Order #${quote.convertedOrderId}.` },
      });
      return;
    }
    ...
    const orderItems: OrderItem[] = quote.items.map(it => ({
      ...
      sellerType: 'ADMIN',
      warrantyMonths: 24,
      warrantyExpiry: new Date(Date.now() + 24 * 30.5 * 24 * 60 * 60 * 1000).toISOString(),
    }));
    ...
    res.status(500).json({ success: false, error: { message: err.message || 'Failed to convert quote to order.' } });
  }
});
